    ...
    if (functionsMissingDocblocks.length > 0) {
      if (mode === MODE_REPORT) {
        ...
        functionsMissingDocblocks.forEach((func) => {
          ...
          console.log(
            `  - Function '${functionName}' at line ${func.loc.start.line} is missing a docblock.`,
          );
        });
        ...
        functionsMissingDocblocks.forEach((func) => {
          ...
          const indentation = lines[functionLine]
            ? lines[functionLine].match(/^\s*/)[0]
            : "";
          /**
           * @description TODO: Add description
        ...
        console.log(
          `Generated docblocks for ${functionsMissingDocblocks.length} functions in ${filePath}`,
        );
      }
    }
  } catch (error) {
